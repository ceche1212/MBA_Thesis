#Heating Degree Days:Heating degree
#days are the number of degrees that the daily average temperature falls below 65
...
data<-read.csv("https://www.eia.gov/totalenergy/data/browser/csv.php?tbl=T01.09")

# Filter data for only total of net imports

data1<-subset(data,data$Description=="Heating Degree-Days, United States")
rm(data)
...
colnames(data1)<-c("Date","Month","Year","HDD","Units")

HDD<-data1

save(HDD,file = "HDD.RData")
