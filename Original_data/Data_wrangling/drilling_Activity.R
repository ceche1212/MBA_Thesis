...
data<-read.csv("https://www.eia.gov/totalenergy/data/browser/csv.php?tbl=T05.01")

# Filter data for only total of active rotary rigs

data1<-subset(data,data$Description=="Crude Oil and Natural Gas Rotary Rigs in Operation, Total")
rm(data)
...
#Rig count values were imported as factors with trailing whitespace which impedes a propper conversion to numeric
...
colnames(data1)<-c("Date","Month","Year","Active_Rigs","Units")

Rigs<-data1

save(Rigs,file = "Rigs.RData")
